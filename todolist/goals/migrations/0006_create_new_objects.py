...
def create_objects(apps, schema_editor):
    ...
    User = apps.get_model("core", "User") #импорт модели на состояние данной миграции
    #этим мы фикируем состояние этих моделей и потом они не меняются
    ...
    #содаем переменную, чтобы всем проставить одинаковое время
    now = timezone.now()


    with transaction.atomic():  # Применяем все изменения одной транзакцией - нам надо или все или нет
        for user_id in User.objects.values_list('id', flat=True):  # Для каждого пользователя
            #вытягиваем только айдишнки
            new_board = Board.objects.create(title="Мои цели",created=now,  # Проставляем вручную по той же причине, что описана вверху
                updated=now)
            BoardParticipant.objects.create(user_id=user_id, board=new_board,role=1,  # Владелец, проставляем числом, не импортируем код по той же причине
                created=now,
                updated=now
            )

            # проставляем всем категориям пользователя его доску
            GoalCategory.objects.filter(user=user_id).update(board=new_board)
